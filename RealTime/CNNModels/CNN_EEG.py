...
class EEGCNNMulticlass(nn.Module):
    def __init__(self, input_features, num_classes, config):
        ...
        self.conv1 = nn.Conv1d(input_features, channels[0], kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm1d(channels[0])
        self.conv2 = nn.Conv1d(channels[0], channels[1], kernel_size=3, padding=1)
        self.bn2 = nn.BatchNorm1d(channels[1])
        self.conv3 = nn.Conv1d(channels[1], channels[2], kernel_size=3, padding=1)
        self.bn3 = nn.BatchNorm1d(channels[2])
        
        # Capa avanzada de atención (para multi-clase)
        if config['attention_mechanism']:
            self.attention = nn.Sequential(
                nn.AdaptiveAvgPool1d(1),
                nn.Conv1d(channels[2], channels[2] // 4, kernel_size=1),
                nn.ReLU(),
                nn.Conv1d(channels[2] // 4, channels[2], kernel_size=1),
                nn.Sigmoid()
            )
        else:
            self.attention = None
        ...
        self.dropout2 = nn.Dropout(dropout_rate + 0.05)  # Más dropout en capas fully connected
        
        # Capas fully connected con dimensiones fijas del modelo guardado
        self.fc1 = nn.Linear(384, hidden_dim)  # Primera capa: 384 -> 384
        self.bn_fc1 = nn.BatchNorm1d(hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, hidden_dim // 2)  # Segunda capa: 384 -> 192
        self.bn_fc2 = nn.BatchNorm1d(hidden_dim // 2)
        self.fc3 = nn.Linear(hidden_dim // 2, num_classes)  # Capa final: 192 -> num_classes
    ...
    def forward(self, x):
        ...
        # Aplicar mecanismo de atención si está habilitado
        ...
        if x.size(1) != 384:
            # Ajustar dimensiones usando interpolación lineal
            x = torch.nn.functional.interpolate(
                x.unsqueeze(1),  # Añadir dimensión de canal
                size=384,  # Tamaño objetivo
                mode='linear'  # Usar interpolación lineal
            ).squeeze(1)  # Remover dimensión de canal
