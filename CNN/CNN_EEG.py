...
class EEGCNNMulticlass(nn.Module):
    def __init__(self, input_features, num_classes, config):
        ...
        self.fc1 = nn.Linear(channels[2], hidden_dim)  # Dimensión inicial, se ajustará dinámicamente
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'fc1') and self.fc1.in_features != x.size(1):
            ...
            self.fc1 = nn.Linear(new_size, CONFIG['hidden_dim']).to(x.device)
            self.bn_fc1 = nn.BatchNorm1d(CONFIG['hidden_dim']).to(x.device)
